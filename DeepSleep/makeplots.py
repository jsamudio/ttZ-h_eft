...
def test_nn_inputs():
    '''
    [0.         0.03883142 0.34237195 0.46185794 0.63760599 0.8111673, 1.        ]# 200 -300
    [0.         0.08306063 0.43137971 0.55986929 0.73463416 0.8649936, 1. ] # 300 -450
    [0.         0.08192864 0.44921828 0.60806015 0.75884196 0.88327139, 1. ] # >450
    '''
    ...
    Hist(processes,'newgenm_NN',       xlabel='NN: with pnet', bins= [0., 0.083, 0.431, 0.560, 0.736, 0.865, 1. ], add_cuts="Zh_pt>300;Zh_M>75;Zh_M<145", doLog=True, doNorm=False, addSoB=True, doCuts=True, doShow=False)
    #StackedHist(processes,'newgenm_NN', xlabel='DNN score', bin_range=[0,1],  n_bins=20,  doCuts=True, addData=False, doShow=True)
    #StackedHist(processes,'Zh_bbvLscore', xlabel='Z/H pnet bbvL', bins=[0.6,.625,.65,.675,.7,.725,.75,.775,.8,.825,.85,.875,.9,.925,.95,.975,1.0],  doCuts=True, addData=False, doLog=True, doShow=True)
    #StackedHist(processes,    'Zh_pt', xlabel=r'Z/H $p_{\mathrm{T}}$ [GeV]', bins=[200,300,450,600],  doCuts=True, doLog=True, addData=False, doShow=True)
    #StackedHist(processes,'Zh_M', xlabel=r'Z/H $m_{\mathrm{SD}}$ [GeV]', bin_range=[50,200],  n_bins=10, doCuts=True,  doLog=True, addData=False, doShow=True)
